...
async function urlToBase64(url: string): Promise<string> {
  ...
  if (!res.ok) {
    throw new Error(`Failed to fetch uploaded media (${res.status})`);
  }
...
export async function POST(
  ...
  try {
    ...
    const content: MediaContentPart[] = [{ type: "text", text: query }];

    if (mediaUrls.length > 0) {
      for (const mediaUrl of mediaUrls) {
        const pathPart = mediaUrl.split("?")[0] || mediaUrl;
        const ext = getExtension(pathPart);
        const supported = SUPPORTED[ext];
        if (!supported) {
          return NextResponse.json(
            {
              error: `Unsupported media type: ${mediaUrl}`,
        ...
        const url = `data:${supported.mime};base64,${base64}`;
        if (supported.contentType === "image_url") {
