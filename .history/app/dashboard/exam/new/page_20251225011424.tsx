...
import { useToast } from "@/hooks/use-toast"; // Fallback to useToast if sonner not found
import {
  ArrowLeft,
  Bolt,
  CloudUpload,
  ...
  Rule,
  EditNote,
} from "@mui/icons-material"; // Wait, I should use lucide-react as per project standards
import {
...
export default function CreateExamPage() {
    ...
    try {
      ...
      const generateRes = await fetch("/api/exams/generate", {
        ...
        body: JSON.stringify({
          ...
          type: values.questionTypes[0], // Simplified for now, backend takes single type primarily or mixed logic
          model: values.model,
        }),
      });
  ...
  return (
    <div className="w-full h-full bg-white dark:bg-[#1a190b] overflow-y-auto p-6 md:p-10">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                Configure your exam settings, upload materials, and let AI generate the questions.
              </p>
            </div>
            <div className="hidden md:block">
              <span className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 dark:bg-surface-dark text-xs font-medium text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-border-dark">
                <span className="w-2 h-2 rounded-full bg-green-500"></span> AI Generator Ready
              </span>
            </div>
          </div>
        </header>
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    ...
                    <Select onValueChange={(val) => form.setValue("subject", val)} defaultValue={form.getValues("subject")}>
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">AI Model</label>
                    <Select onValueChange={(val) => form.setValue("model", val)} defaultValue={form.getValues("model")}>
                       <SelectTrigger className="w-full bg-transparent border-slate-200 dark:border-slate-800 rounded-xl h-12">
                        <SelectValue placeholder="Select Model" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="llama-3.1-8b-instant">Fast (Llama 3.1 8b)</SelectItem>
                        <SelectItem value="llama-3.3-70b-versatile">Advanced (Llama 3.3 70b)</SelectItem>
                      </SelectContent>
                    </Select>
                     <p className="text-[10px] text-slate-400 mt-1.5 flex items-center gap-1">
                        <InfoIcon className="w-3 h-3" />
                        Advanced creates more complex reasoning questions.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-surface-light dark:bg-slate-900 border border-border-light dark:border-border-dark rounded-3xl p-6 md:p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Number of Questions</label>
               ...
               <div>
                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-3">Question Types</label>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {[
                    { id: "mcq", label: "Multiple Choice", icon: CheckBoxIcon },
                    { id: "true_false", label: "True / False", icon: RuleIcon },
                    { id: "short_answer", label: "Short Answer", icon: EditNoteIcon },
                  ].map((type) => {
                        ...
                        >
                             <div className="flex justify-between items-start">
                                ...
                                <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${
                                    ...
                                    {isSelected && <CheckIcon className="w-3 h-3 text-slate-900" />}
                                </div>
                             </div>
                             <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{type.label}</span>
                        </div>
                      )
                  })}
                </div>
            ...
            <div className="bg-surface-light dark:bg-slate-900 border border-border-light dark:border-border-dark rounded-3xl p-6 md:p-8">
               <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Source Material</h3>
              </div>
               ...
               <div className="flex gap-2 mb-6 border-b border-border-light dark:border-border-dark">
                ...
                <button 
                    ...
                    className={`pb-3 px-4 text-sm font-medium border-b-2 transition-colors ${
                         activeTab === "text" ? "border-primary text-primary" : "border-transparent text-slate-500 hover:text-slate-800"
                    }`}
                >
                    Paste Text
                </button>
               ...
               {activeTab === "upload" ? (
                  <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl p-8 flex flex-col items-center justify-center text-center bg-slate-50 dark:bg-white/5 hover:border-primary hover:bg-primary/5 transition-all cursor-pointer group h-64">
                      ...
                      <h4 className="text-base font-bold text-slate-900 dark:text-white mb-1">Click to upload or drag & drop</h4>
                       ...
                       <Button variant="outline" className="mt-4" onClick={() => toast({ title: "Coming Soon", description: "File upload is WIP. Please use 'Paste Text' tab." })}>
                           Browse Files
                       </Button>
                  </div>
               ) : (
                   <div>
                       ...
                       />
                       <p className="text-right text-xs text-slate-400 mt-2">{form.watch("sourceText")?.length || 0} characters</p>
                   </div>
               )}
            </div>
          ...
          <div className="lg:col-span-1">
             <div className="sticky top-6 space-y-6">
                 <div className="bg-surface-light dark:bg-slate-900 border border-border-light dark:border-border-dark rounded-3xl p-6 shadow-xl shadow-slate-200/50 dark:shadow-none">
                     <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Exam Summary</h3>
                     <ul className="space-y-3 mb-6">
                        <li className="flex justify-between items-center text-sm">
                            <span className="text-slate-500 dark:text-slate-400">Questions</span>
                            <span className="font-bold text-slate-900 dark:text-white">{form.watch("questionCount")} Qs</span>
                        </li>
                         <li className="flex justify-between items-center text-sm">
                            <span className="text-slate-500 dark:text-slate-400">Duration</span>
                            <span className="font-bold text-slate-900 dark:text-white">{form.watch("duration")} mins</span>
                        </li>
                         <li className="flex justify-between items-center text-sm">
                            ...
                            <span className="font-bold text-slate-900 dark:text-white truncate max-w-[100px]">
                                {watchQuestionTypes.map(t => t === 'mcq' ? 'Multiple Choice' : t === 'true_false' ? 'True/False' : 'Short Answer').join(", ")}
                            </span>
                        </li>
                         <li className="flex justify-between items-center text-sm">
                            <span className="text-slate-500 dark:text-slate-400">Model</span>
                            <span className="font-bold text-slate-900 dark:text-white">
                                {form.watch("model").includes("70b") ? "Advanced" : "Fast"}
                            </span>
                        </li>
                        <li className="pt-3 border-t border-dashed border-slate-200 dark:border-slate-700 flex justify-between items-center text-sm">
                             <span className="text-slate-500 dark:text-slate-400">Est. Gen Time</span>
                          ...
                          <div>
                            <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-1">Pro Tip</h4>
                             <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
                                Uploading focused notes yields better results than entire textbooks. Try to limit content to specific chapters.
                             </p>
                          </div>
                    </div>
                 </div>
