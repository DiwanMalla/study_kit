...
export function ExamTaker({ exam }: ExamTakerProps) {
  ...
  const latestAttempt = Array.isArray(exam.attempts)
    ? exam.attempts[0]
    : undefined;
  ...
  const [examComplete, setExamComplete] = useState(
    !!latestAttempt || (exam.score !== null && exam.score !== undefined)
  );
  const [timeLeft, setTimeLeft] = useState(() => {
    const total = exam.duration * 60;
    if (latestAttempt && typeof latestAttempt.timeSpentSeconds === "number") {
      return Math.max(0, total - Math.max(0, latestAttempt.timeSpentSeconds));
    }
    return total;
  });
  ...
  useEffect(() => {
    ...
    // Prefer restoring from the latest saved attempt (server-provided) so results work across devices.
    if (
      latestAttempt &&
      Array.isArray(latestAttempt.answers) &&
      latestAttempt.answers.length > 0
    ) {
      setAnswers(new Map(latestAttempt.answers));

      if (typeof latestAttempt.timeSpentSeconds === "number") {
        const total = exam.duration * 60;
        const nextTimeLeft = Math.max(
          0,
          total - Math.max(0, latestAttempt.timeSpentSeconds)
        );
  ...
  }, [exam.id, examComplete, latestAttempt]);
  ...
  const handleSelect = (optionIndex: number) => {
    ...
    // Persist immediately so the question navigator shows this as answered
    // even if the user jumps to another question without clicking "Next".
    setAnswers((prev) => {
  ...
  const isShortAnswerQuestion = (q: ExamQuestion) =>
    q.type === "short_answer" || q.type === "short_essay";
  ...
  const isAnswerCorrect = (
    ...
    if (isShortAnswerQuestion(q)) {
      ...
      return (
        normalizeAnswer(userAnswer) === normalizeAnswer(String(correctText))
      );
    }
  ...
  const calculateScore = (answerMap: Map<number, number | string>) => {
    let correct = 0;
  ...
  const persistAttempt = (
    answerMap: Map<number, number | string>,
    timeSpentSeconds: number
  ) => {
    if (typeof window === "undefined") return;
    try {
  ...
  const handleFinishExam = async () => {
    ...
    const finalAnswers = new Map<number, number | string>(answers);
    ...
    try {
      const res = await fetch(`/api/exams/${exam.id}/attempts`, {
        ...
        body: JSON.stringify({
          answers: Array.from(finalAnswers.entries()),
          timeSpentSeconds,
        }),
      });
      ...
      // Ensure server state (latest attempt) is reflected after submit.
      router.refresh();
    } catch (error) {
  ...
  if (examComplete) {
    ...
    const correctCount = hasSavedAnswers
      ? Array.from(answers.entries()).filter(([idx, ans]) => {
          const q = exam.questions[idx];
          return q ? isAnswerCorrect(q, ans) : false;
        }).length
              ...
              <div className="bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-[2rem] shadow-xl shadow-slate-200/40 dark:shadow-none p-8 md:p-12 max-w-2xl w-full text-center relative overflow-hidden">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
                  ...
                  <div className="p-4 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 flex flex-col items-center">
                    ...
                    <span className="text-xl font-mono font-bold text-slate-900 dark:text-white">
                      #{(latestAttempt?.id ?? exam.id).slice(-5).toUpperCase()}
                    </span>
                  </div>
                </div>
    ...
    const skippedCount = hasSavedAnswers
      ? exam.questions.length - answers.size
      : 0;
                  ...
                  currentQuestion.type === "short_essay" ? (
                    <div className="space-y-4">
                      <textarea
                        ...
                        value={
                          typeof answers.get(currentIndex) === "string"
                            ? (answers.get(currentIndex) as string)
                            : ""
                        }
                        onChange={(e) => {
                          ...
                          if (nextValue.trim().length === 0) {
                            ...
                            newAnswers.set(currentIndex, nextValue);
                          }
            ...
            <div className="bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-[2rem] p-6 flex flex-col h-full shadow-lg shadow-slate-200/50 dark:shadow-none max-h-[calc(100vh-140px)] lg:max-h-none overflow-hidden">
              ...
              <div className="grid grid-cols-5 gap-3 content-start flex-1 overflow-y-auto custom-scrollbar pr-2">
                {exam.questions.map((_, idx) => {
                  const answerValue = answers.get(idx);
                  const isAnswered =
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setSelectedAnswer(
                          typeof nextAnswer === "number" ? nextAnswer : null
                        );
                      }}
                    >
                      {idx + 1}
