...
export function ExamTaker({ exam }: ExamTakerProps) {
  ...
  const latestAttempt = Array.isArray(exam.attempts) ? exam.attempts[0] : undefined;
  ...
  useEffect(() => {
    ...
    if (latestAttempt && Array.isArray(latestAttempt.answers) && latestAttempt.answers.length > 0) {
      setAnswers(new Map(latestAttempt.answers));

      if (typeof latestAttempt.timeSpentSeconds === "number") {
        const total = exam.duration * 60;
        const nextTimeLeft = Math.max(0, total - Math.max(0, latestAttempt.timeSpentSeconds));
          ...
          <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8">
            <div>
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                Completed on {latestAttempt?.createdAt ? new Date(latestAttempt.createdAt).toLocaleDateString() : new Date().toLocaleDateString()} •{" "}
                {formatTimeSpent(totalSecondsSpent)}
              </p>
            </div>
