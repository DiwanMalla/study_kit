...
export function ExamList({ initialExams }: { initialExams: Exam[] }) {
  ...
  const avgScore =
    completed.length > 0
      ? Math.round(
          completed.reduce((acc, curr) => acc + (curr.score || 0), 0) /
            completed.length
        )
      : 0;
  const totalTime = completed.reduce(
    ...
    0
  ); // Mock duration accumulation

  return (
    <div className="max-w-4xl mx-auto flex flex-col h-full pt-8">
      {/* Header */}
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        ...
        <div>
          <Link href="/dashboard/exam/new">
            <Button className="px-6 py-3 bg-primary text-white rounded-xl font-bold shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 hover:-translate-y-1 active:translate-y-0 active:shadow-md transition-all flex items-center gap-2">
      ...
      {filteredExams.length > 0 ? (
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-1 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      {exam.status !== "completed" && (
                        <button
                          className="p-2 hover:bg-red-100 dark:hover:bg-red-900/20 rounded-full text-slate-400 hover:text-red-500 transition-colors disabled:opacity-50"
                          title="Delete"
                        ...
                        >
                          <Trash2
                            className={`w-4 h-4 ${
                              isDeleting === exam.id ? "animate-pulse" : ""
                            }`}
                          />
                        </button>
                      )}
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center bg-white dark:bg-surface-dark rounded-[2.5rem] border border-border-light dark:border-border-dark shadow-xl shadow-slate-200/40 dark:shadow-none">
          <div className="w-24 h-24 bg-slate-100 dark:bg-white/5 rounded-[2rem] flex items-center justify-center mb-8">
          ...
          <p className="text-slate-500 dark:text-slate-400 max-w-md mb-10 mx-auto">
            You haven't created any mock exams yet. Start by creating one to
            test your knowledge and track your progress.
          </p>
          <Link
            href="/dashboard/exam/new"
            className="px-10 py-4 bg-primary text-white rounded-2xl font-bold shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 hover:-translate-y-0.5 active:translate-y-0 transition-all flex items-center gap-2"
          >
