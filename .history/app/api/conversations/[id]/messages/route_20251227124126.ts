...
2. Includes all specific details, labels, and requirements
...
Output ONLY the enhanced prompt as plain text. No JSON, no markdown, no commentary.`,
                      },
                      {
                        role: "user",
                        content: `Conversation Context:\n${conversationHistory}\n\nUser Request: ${message}\n\nGenerate a detailed, context-aware image prompt:`,
                      },
                    ],
                    temperature: 0.7,
                    max_tokens: 500,
              ...
              if (promptEngineeringResponse.ok) {
                ...
                if (generatedPrompt) {
                  ...
                  console.log("Optimized Image Prompt from Kimi K2:", optimizedPrompt);
                }
              } else {
                console.error("OpenRouter request failed:", await promptEngineeringResponse.text());
              }
            } catch (err) {
              console.error("Failed to optimize prompt with Kimi K2:", err);
              // Fallback to basic cleaning if Kimi fails
              ...
              // If the prompt is empty, use the last user message for context
