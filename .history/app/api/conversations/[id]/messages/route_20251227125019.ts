...
Output ONLY the enhanced prompt as plain text. No JSON, no markdown, no commentary. Just the prompt itself.`;
              ...
              try {
                ...
                if (groqResponse.ok) {
                  ...
                  groqPrompt = response?.choices?.[0]?.message?.content?.trim() || "";
              ...
              try {
                ...
                if (openrouterResponse.ok) {
                  ...
                  openrouterPrompt = response?.choices?.[0]?.message?.content?.trim() || "";
                  if (openrouterPrompt && openrouterPrompt.length > 20) {
                    console.log("✓ OpenRouter Kimi K2 Prompt:", openrouterPrompt);
                  }
                } else {
