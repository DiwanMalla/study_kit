...
function parseSelectedModel(value: unknown): {
  ...
  if (trimmed === "auto") trimmed = "nvidia:nvidia/nemotron-3-nano-30b-a3b";
      ...
      async start(controller) {
        try {
          if (provider === "gemini") {
            ...
            // Fallback if Nemotron failed or returned empty
            if (!optimizedPrompt || optimizedPrompt.length < 5) {
              ...
              // If the cleaned prompt is very short or generic (like "about it"), use the last message
              if (
                !cleaned ||
                cleaned.length < 3 ||
                /^(about it|it|that|this|the same|above)$/i.test(cleaned)
              ) {
                const lastUserMsg = conversation.messages
                  .filter((m) => m.role === "user")
                  .pop();
                optimizedPrompt = lastUserMsg
                  ? `${lastUserMsg.content} ${cleaned}`.trim()
                  : message;
              } else {
                ...
                messages: [
                  {
                    role: "system",
                    content:
                      "You are an expert image prompt engineer. Your task is to take a user's request and the conversation context, and generate a highly detailed, descriptive, and artistic prompt for an image generation model (like Qwen-Image). Focus on visual details, lighting, style, and composition. Output ONLY the optimized prompt text, nothing else.",
                  },
                  {
                    role: "user",
                    content: `Conversation Context:\n${conversationHistory}\n\nUser Request: ${message}\n\nGenerate a detailed image prompt:`,
