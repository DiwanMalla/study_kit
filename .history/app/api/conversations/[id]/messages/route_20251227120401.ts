...
function parseSelectedModel(value: unknown): {
  ...
  if (trimmed.startsWith("nvidia:") || trimmed.includes("stabilityai/stable-diffusion")) {
     return {
      provider: "nvidia",
      modelId:
        trimmed.replace("nvidia:", "").trim() || "stabilityai/stable-diffusion-3.5-large",
    };
  }
          ...
          if (provider === "gemini") {
            ...
            flushSmooth(controller);
             ...
             flushSmooth(controller);
              ...
              const imageUrl = await qwenImageGeneration(cleanedPrompt, modelId || "qwen-image-plus");
              const markdown = `![Generated Image](${imageUrl})`;
              fullResponse = markdown;
              enqueueSmooth(controller, markdown);
              flushSmooth(controller);
