...
function parseSelectedModel(value: unknown): {
  ...
  const [providerRaw, modelIdRaw] = value.split(":", 2);
  const provider = providerRaw === "groq" ? "groq" : "gemini";
  const modelId = (modelIdRaw || "gemini-2.5-flash").trim();
...
export async function POST(
  ...
  try {
    ...
    const { provider, modelId } = parseSelectedModel(model);
...
Please provide a helpful, clear response.`;
    ...
    const enqueueSmooth = (controller: ReadableStreamDefaultController, text: string) => {
      wordBuffer += text;
    ...
    // Build request for Groq up-front (so we can fail early with JSON response)
        ...
        return NextResponse.json(
          {
            ...
            details: details || groqResponse.statusText,
          },
          { status: 500 }
        );
      }
      ...
      async start(controller) {
        try {
          if (provider === "gemini") {
            ...
            if (!body) {
              throw new Error("Groq streaming body missing");
            }
