      ...
      async start(controller) {
        try {
          if (provider === "gemini") {
            ...
            // Use Kimi K2 (via OpenRouter) as an Image Prompt Compiler
            ...
            try {
              ...
              // Fetch from OpenRouter using Kimi K2
              const promptEngineeringResponse = await fetch(
                "https://openrouter.ai/api/v1/chat/completions",
                {
                  method: "POST",
                  headers: {
                  ...
                  body: JSON.stringify({
                    ...
                    messages: [
                      {
                        role: "system",
                        content: `You are an expert image prompt generator. Based on the conversation context, create a detailed, professional image prompt that:
...
Output ONLY the enhanced prompt as plain text. No JSON, no markdown, no commentary. Just the prompt itself.`,
              ...
              if (promptEngineeringResponse.ok) {
                const response = await promptEngineeringResponse.json();
                const generatedPrompt =
                  response?.choices?.[0]?.message?.content?.trim();
                if (
                  generatedPrompt &&
                  generatedPrompt.length > 20 &&
                  !generatedPrompt.toLowerCase().includes("error")
                ) {
                  optimizedPrompt = generatedPrompt;
                  console.log(
                    "Optimized Image Prompt from Kimi K2:",
                    optimizedPrompt
                  );
                } else {
                  console.warn("Invalid response from Kimi K2, using fallback");
                  throw new Error("Invalid response");
                }
              } else {
                const errorText = await promptEngineeringResponse.text();
              ...
              // Smarter fallback: extract from the last AI response about the topic
              const lastAIResponse = conversation.messages
                .reverse()
                .find((m) => m.role === "assistant");

              if (lastAIResponse && lastAIResponse.content.length > 100) {
                // Extract first meaningful sentence as context
                const sentences = lastAIResponse.content.split(/\.\s+/);
                optimizedPrompt = sentences
                  .slice(0, 3)
                  .join(". ")
                  .slice(0, 500);
                console.log(
                  "Using context from previous AI response:",
                  optimizedPrompt
                );
              } else {
                // Final fallback: just use the last meaningful user message
                const lastUserMsg = conversation.messages
                  .filter((m) => m.role === "user")
                  .slice(-2)[0]; // Get second-to-last user message (the topic)

                if (lastUserMsg && lastUserMsg.content.length > 20) {
                  optimizedPrompt = lastUserMsg.content;
                  console.log(
                    "Using context from user question:",
                    optimizedPrompt
                  );
                }
              }
