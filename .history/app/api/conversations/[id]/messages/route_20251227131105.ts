...
export async function POST(
  ...
  try {
    ...
    // Check if user is asking for image generation - support various phrasings
    const imageTrigger =
      lowerMessage.startsWith("image:") ||
      lowerMessage.startsWith("/image") ||
      lowerMessage.startsWith("img:") ||
      lowerMessage.startsWith("generate image:") ||
      // Match patterns like "generate an image", "can you generate it", "again", "once more", etc.
      ...
      // Match follow-up requests
      /\b(again|once more|one more time|re-?generate|retry|redo)\b/i.test(
        lowerMessage
      ) ||
      // Match simple "can you generate it" style requests
      /\bgenerate\b.*\b(it|that)\b/i.test(lowerMessage) ||
      /\bcan you.*\b(generate|create|draw|make|show)\b/i.test(lowerMessage);
        ...
        try {
          if (provider === "gemini") {
            ...
            try {
              ...
              const systemPrompt = `You are an expert image prompt generator. Your task is to take a user's request and the conversation context to create a highly detailed, professional, and STRUCTURED image prompt.

CRITICAL RULES:
1. PRESERVE AND REPLICATE STRUCTURE: If the user provides a structured request (with headers, bullet points, numbered steps, or sections like "Title:", "Layout:", "Callout labels:"), YOU MUST MAINTAIN AND ENHANCE THIS STRUCTURE. Do not collapse it into a single paragraph.
2. PRESERVE ALL KEYWORDS: Keep every specific label, technical term, and style description (e.g., "Gray's Anatomy plates", "ECG trace", "Organ of Corti", "Basilar Membrane").
3. EXPAND TECHNICAL DETAIL: For each section, add specific technical descriptors that help an image generator (e.g., for "Cochlea", add "cross-section showing scala vestibuli and scala tympani, fluid dynamics").
4. FORMATTING: Use clear headers, newlines, and bullet points. The output should look like a professional technical specification for an artist.
5. Output ONLY the enhanced prompt. No preamble, no "Here is your prompt", no commentary.
6. The final output should be extremely detailed, likely 200-400 words if the input is complex.`;
              ...
              // 2. Fetch from OpenRouter using Gemini 2.0 Flash
              try {
              ...
              if (groqPrompt && groqPrompt.length > 20) {
                ...
                throw new Error("Both prompt optimization providers failed");
              }
              ...
              console.error(
                "Failed to optimize prompt, using context fallback:",
                err
              );
