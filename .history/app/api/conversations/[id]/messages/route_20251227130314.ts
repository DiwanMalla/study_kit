...
2. EXPAND on the details to help the image generator, but do not contradict the user's intent.
3. MAINTAIN the requested atmosphere and educational/professional tone.
4. Output ONLY the enhanced prompt as plain text. No preamble, no commentary, no markdown.
5. The final prompt should be descriptive and at least 60-80 words long.
...
Example of good expansion: If user asks for "heart tissue", you might add "detailed muscular fibers, coronary arteries, ventricles and atria clearly visible, high resolution, medical illustration style".`;
              ...
              // 1. Fetch from Groq using Llama 3.3 70B
              try {
                ...
                if (groqResponse.ok) {
                  ...
                  if (groqPrompt && groqPrompt.length > 20) {
                    console.log("✓ Groq Llama 3.3 Prompt:", groqPrompt);
                  }
                } else {
