...
import { qwenImageGeneration, nvidiaChatCompletion } from "@/lib/ai";
      ...
      async start(controller) {
        try {
          if (provider === "gemini") {
            ...
            // Use NVIDIA Nemotron as an Image Prompt Compiler
            ...
            try {
              const promptEngineeringResponse = await nvidiaChatCompletion({
                ...
                messages: [
                  {
                    role: "system",
                    content: `You are an Image Prompt Compiler. Your job is to preserve the user's intent and structure with minimal edits.

Rules:
1. Do NOT change the requested style (e.g., "medical infographic", "blueprint", "vector diagram"). Never replace it with "cinematic", "hyper-realistic macro", "artistic", etc.
2. Do NOT shorten or truncate. Keep all required labels exactly as provided.
3. If the user requests readable text/labels, keep them and add: "clear readable English text, no gibberish".
4. Do NOT add extra commentary or markdown.
5. Output must be ONLY valid JSON with keys: "prompt" (string), "negative_prompt" (string), "notes" (string, optional).

Based on the conversation context, enhance the user's image request while preserving their exact intent and style preferences.`,
                  },
                  {
                    role: "user",
                    content: `Conversation Context:\n${conversationHistory}\n\nUser Request: ${message}\n\nCompile and enhance the image prompt while preserving the user's intent. Return ONLY valid JSON with keys: prompt, negative_prompt, notes.`,
                  },
                ],
                temperature: 0.5,
                max_tokens: 300,
              ...
              if (generatedContent) {
                try {
                  ...
                  optimizedPrompt = parsed.prompt || message;
                  negativePrompt = parsed.negative_prompt || "";
                  console.log("Optimized Image Prompt:", optimizedPrompt);
                } catch {
                  // If not valid JSON, treat as raw prompt
                  optimizedPrompt = generatedContent;
                }
              }
            } catch (err) {
              ...
              // If the prompt is empty or just a generic request, try to use the last user message for context
