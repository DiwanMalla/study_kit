...
import { nvidiaImageGeneration, qwenImageGeneration } from "@/lib/ai";
      ...
      async start(controller) {
        try {
          if (provider === "gemini") {
            ...
            // Send a "Generating image..." status if possible?
            // For now just wait.
            const imageUrl = await nvidiaImageGeneration(message, modelId);
            const markdown = `![Generated Image](${imageUrl})`;

            // Stream the markdown result
            fullResponse = markdown;
            // Split into chunks to simulate typing? No, just one chunk for image is fine or small chunks.
            // enqueueSmooth handles splitting by buffer.
            ...
            let cleanedPrompt = message
              .replace(/^image:\s*/i, "")
              .replace(/^img:\s*/i, "")
              .replace(/^\/image\s*/i, "")
              .replace(/^generate image:\s*/i, "")
              .replace(/can you (generate|create|show me|make|draw) (an image|a picture) of/i, "")
              .replace(/(generate|create|show me|make|draw) (an image|a picture) of/i, "")
              .replace(/can you (generate|create|show me|make|draw) (an image|a picture)/i, "")
              .replace(/(generate|create|show me|make|draw) (an image|a picture)/i, "")
              .trim();
            ...
            if (!cleanedPrompt || cleanedPrompt.length < 3) {
              const lastUserMsg = conversation.messages
                ...
                .pop();
              if (lastUserMsg) {
                cleanedPrompt = lastUserMsg.content;
              } else {
            ...
            const markdown = `Here is the image you requested for "${cleanedPrompt}":\n\n![Generated Image](${imageUrl})`;
            fullResponse = markdown;
            enqueueSmooth(controller, markdown);
            flushSmooth(controller);
          } else {
