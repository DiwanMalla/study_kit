...
function isNvidiaModel(model: string): boolean {
  ...
  const nvidiaModels = AI_MODELS.filter(m => m.provider === "NVIDIA").map(m => m.id);
 ...
 */
export async function nvidiaImageGeneration(prompt: string, model: string = "stabilityai/stable-diffusion-3.5-large"): Promise<string> {
  if (!process.env.NVIDIA_API_KEY) {
  ...
  const url = "https://integrate.api.nvidia.com/v1/genai/stabilityai/stable-diffusion-3.5-large";
