...
if (!process.env.GEMINI_API_KEY) {
  ...
  console.warn("GEMINI_API_KEY is not set in environment variables. Gemini functionality might be limited.");
}
 ...
 */
export async function generateSummary(content: string, modelType: ModelType = "auto"): Promise<string> {
...
export async function generateAssignmentSolution(
 ...
 `;
  ...
  try {
    ...
    return completion.choices[0]?.message?.content || "Failed to generate solution.";
  } catch (error) {
