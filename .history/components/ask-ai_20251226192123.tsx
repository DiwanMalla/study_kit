...
export function EnhancedAskAI({ enabledModels }: EnhancedAskAIProps) {
  ...
  const handleMediaSelected = async (
    ...
    if (hasVideo && files.length !== 1) {
      setMessages((prev) => [
        ...prev,
        {
          id: `error-${Date.now()}`,
          role: "assistant",
          content: "Only a single video is supported. Please select one video, or upload multiple images.",
          createdAt: new Date(),
        },
      ]);
      return;
    ...
    const query = (input && input.trim().length > 0 ? input : "Describe the scene").trim();
    setInput("");
